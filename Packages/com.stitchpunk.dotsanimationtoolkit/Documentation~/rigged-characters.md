...
- **The space is centred on 0,0,0.** Two grids: a backdrop in the XY plane to measure a flat rig against, and a floor in the XZ plane so a 3D prop or vehicle has something to stand on. The origin itself is drawn as three short axis stubs — X red, Y green, Z blue. The rig is spawned there and the camera orbits it, so a character authored standing on the floor reads as standing on the floor. Double-click the viewport to return the camera to it.
