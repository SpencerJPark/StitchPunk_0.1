...
**Window ▸ DOTS Animation Toolkit ▸ DOTS Animator** — the Flipbooks tab.
...
own number. The first Save writes a small `<ArrayName>_Flipbook.asset` beside
the array, holding only the frame names — nothing about the array itself
changes, and no material is touched.

A **●** marks unsaved name changes, and switching flipbooks while it shows asks
before discarding.

## Re-importing an array

If the source PNG is re-imported with more layers, opening the flipbook again
...
## Using a flipbook in the Clip Editor

On a sprite track, the **Flipbook** field accepts either a saved flipbook or
a `Texture2DArray` directly. Dropping in an array reuses its existing names
flipbook if one exists, or creates one. The **Frame** and **Base Frame**
...
- The raw **Index** field stays visible but read-only while a flipbook is
  bound.
- `AtlasRect` tracks show the Flipbook field disabled, labelled "flipbooks bind
  Slice tracks" — atlas rects are a different addressing scheme and a flipbook
  has nothing to bind there.

The flipbook reference is authoring-only and is never baked into clip data — a
...
Bake composes the frames into one grid PNG, `T_<FlipbookName>_Array.png`,
written beside the flipbook by default or to the path chosen with the **…**
...
it stay put. The flipbook asset itself is written only by **Save** — a **●**
marks unsaved changes, and switching flipbooks while it shows asks before
discarding.
...
max size 2048, no crunch — with the flipbook's own Filter, Wrap, Mips, and
Linear fields layered on top. There is no uncompressed option.

## Contact flipbook zoom
