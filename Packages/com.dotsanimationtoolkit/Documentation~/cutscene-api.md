...
| `CutscenePlay` | The identity half of a running request: which blob, and which `PlaybackLayer` index (`layerIndex`) clip blocks target on every bound actor. `CutsceneApi.TopLayer` (the default) resolves per actor to that actor's own last layer rather than a fixed index. Immutable once created. | `CutsceneApi.CreatePlayRequest` | `CutsceneTimelineSystem`, `CutscenePartOverrideSystem` | Added at request creation; never removed by the toolkit — destroying the request entity is the host's job. |
...
|---|---|---|
| `CreatePlayRequest` | `static Entity CreatePlayRequest(EntityManager entityManager, BlobAssetReference<CutsceneBlob> blob, byte layerIndex = CutsceneApi.TopLayer, float speed = 1f)` | Creates a request entity: `CutscenePlay`, a fresh `CutsceneControl`, zeroed `CutscenePlaybackState`, an empty `CutsceneActorBinding` buffer, a disabled `CutsceneHoldRelease`, an `AnimEventOutput` buffer + disabled `AnimEventsPending` (same output shape a clip's own events use, scoped to the request entity rather than any one actor), and the internal `CutsceneSlotRuntimeState` buffer pre-sized to the blob's slot count. The player never disposes the blob. The host must still fill `CutsceneActorBinding` before anything plays. |
| `TopLayer` | `const byte TopLayer = byte.MaxValue` | Default `layerIndex` for both request-builders. `CutsceneTimelineSystem` resolves it per bound actor to that actor's own last `PlaybackLayer` index (`playbackLayers.Length - 1`), so the cutscene layer is always the actor's top one regardless of how many layers a given rig has. A literal index is still used exactly as given. |
...
| `CreatePlayRequestFromStage` | `static Entity CreatePlayRequestFromStage(EntityManager entityManager, Entity stageEntity, byte layerIndex = CutsceneApi.TopLayer, float speed = 1f)` | Same as `CreatePlayRequest`, using `stageEntity`'s `CutsceneStage.blob`, plus every `CutsceneStageBinding` on it copied into the new request's `CutsceneActorBinding`. The host may still add entries for actors the stage never baked or that spawned at runtime. |
