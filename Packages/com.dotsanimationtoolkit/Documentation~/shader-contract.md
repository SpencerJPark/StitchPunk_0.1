...
**The packing is confirmed in source** — the bake runner that produced the reference demo mesh packs `uv1` exactly like this:
...
- ~~The `uv1` bake runner could not be found.~~ **Resolved** — its packing is quoted in §4 above.
  The real finding underneath the false alarm is a *packaging* one: that runner lives in the demo
  project rather than in this package, **so the package ships no helper for packing `uv1`**. If you
  are producing meshes yourself, §4 is the contract you have to satisfy by hand.
