...
**New Rig**, beside the toolbar's **Rig** field, builds a `RigAsset` from a prefab instead of assembling one field at a time:
...
4. With a clip set open and the panel's toggle checked, the new rig is assigned into that set's **Rig** field on the way out — the same undoable write a manual pick makes.
