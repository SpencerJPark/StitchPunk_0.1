...
- **The rig hierarchy is the bone picker.** Pick a `RigAsset` in the **Rig** field at the top of the Rig Hierarchy pane — window state, written to no asset, and independent of the **Clip Set** field at the top of the Clips pane: swapping either never changes the other — and its **Source Prefab**'s transforms appear as a tree in the left column. Both fields are shared by every tab: pick a set on **Clip Sets** or a rig on **Rigs** and it takes hold here too, and changing it here changes it there, on **VAT Bake**, and on **Actor Profiles**. Select a bone there and the inspector offers **Add Bone Track** for exactly that bone — no typing, so the "name resolved to nothing and the bake froze it at rest" failure cannot happen. Bones the selected clip already animates are shown in bold. A rig with no Source Prefab set yet (or with nothing assigned at all) leaves the tree empty, but you can still type a bone name by hand, which is the only option a cutout set has. Build a new rig from a prefab on the **Rigs** tab instead of assigning one field at a time.
...
- **Imported clips add read-only lanes.** Under the authored rows, each imported source clip contributes one row per animated bone or node, named after it, drawn with hollow, dimmed keys — they cannot be selected or dragged. A key at imported time k plays at clip time k: keys are placed against the clip's own duration, not the imported clip's length, because that is the duration the runtime plays the baked frames over. Keys past the duration never play, and the row's tooltip counts them. Under a focus selection they hide like authored rows and count into "track(s) hidden".
...
- **Parts start where the prefab puts them.** Pick a rig in the **Rig** field at the top of the Rig Hierarchy pane, and set that rig's **Source Prefab**, and each rig target picks up the position, rotation and scale of the prefab transform with the same name, measured relative to the prefab root. That is the part's rest pose, and a clip animates *from* it — position and rotation add to it, scale multiplies it — which is exactly how the runtime composes, so what you preview is what plays. A target with no matching transform in the prefab falls back to the origin at unit scale.
- **A rig with nothing to move previews anyway, and says why.** If the open rig has no parts, or nothing in the open clip set keys a bone or names a VAT source, it still previews like any other rig — there is just nothing to see move. The status line spells that out: "Rig 'X' has no parts, and nothing in this set keys bones or names a VAT source — nothing here will move." That is information, not a validation error.
...
- **VAT binding.** On a hierarchy node the rig's source prefab bakes as a VAT part, add **VAT binding** from the component stack: a **Source** field (an imported `AnimationClip`; leave it empty and the bake poses the authored bone tracks instead) and a **Loop safe** toggle (appends a copy of frame 0 so a loop has no seam). A line warns when the source clip's length and the clip's duration disagree by more than half a frame — "the last 0.20 s never plays" or "holds its last frame for 0.20 s". It writes the clip's per-part VAT row, or its single untargeted VAT source on a one-mesh rig; nothing here is typed as a number.
- **Validation.** The Health tab carries it. The tab reads **Health (n)** in red while there are errors, and a committed clip edit rescans within about a second, so a binding the bake would reject shows there as H11 with the bake's own rule code. Open the tab for the explanation and the fixes; see [`health-tab.md`](health-tab.md).
...
- **Baked VAT.** The viewport rail's **Baked VAT** toggle — a quad with one lit cell — is off by default and disabled when the open clip set has no VAT texture set. Switch it on and each baked part's live skinned mesh hides while its baked mesh plays at the playhead's frame, drawn at the skeleton's root (the bake records root space), so scrubbing moves baked and cutout parts together. Switch it off and the live skinned meshes come back. Use it to catch bake drift — a stale bake, or a rig changed since the last one.
...
**New**, on the **Rigs** tab, builds a `RigAsset` from a prefab instead of assembling one field at a time. Nothing is torn down when you switch to another tab and back, so a prefab you have already scanned and the nodes you have already ticked survive a trip away and a return:

1. Assign the **Source Prefab** you want the rig built from.
2. The tab scans it for every renderer-bearing node and lists them as candidates, ticked or not — a disabled renderer or an inactive helper object starts unticked, everything else starts ticked, and nothing is ticked for you that you cannot untick. Ticking a node adds it as a rig target.
...
4. The new rig joins the tab's catalog. Clicking any rig there makes it the editor's rig — the same undoable write a manual pick makes, and it takes hold in the **Rig** field on every tab.
...
- **Prefab**, beside **Edit** in the Rig Hierarchy pane's header, opens the loaded prefab. It is enabled only while the **Rig** field holds a rig whose **Source Prefab** is set; with either one missing it says so on hover rather than failing after the press.
...
The window also carries **Rigs**, **VAT Bake**, **Actor Editor**, **Cutscene Editor** and several
other tabs alongside the Clip Editor timeline. The Actor Editor is where a profile's layers, named
