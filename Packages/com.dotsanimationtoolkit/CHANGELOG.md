...
### Changed
- **Ragdoll** now sits right after **Actor Profiles** in the Clip Editor's tab strip, ahead of Cutscenes and Capture. `ClipEditorTab` declarations follow the strip; every tab number is unchanged, so a docked or reloaded window keeps its active tab.
...
- Actor Editor: a collapsible **Layer Events** strip under the Preview column's transport. One row
  per layer shows the playing clip's event pins, a playhead and `t / duration`; a stopped layer's
  row is dimmed and a crossfading-out clip's pins are hollow. Crossings while playing or stepping
  flash the pin and play the event's preview sound. The expanded state is remembered per machine.
- `ActorPreviewComposer.LayerCount` and `ActorPreviewComposer.Layer(int)`, returning a copy of a
  layer's `PlaybackLayer`.
- `LayerEventRowResolver`, the pure row model: whether a layer emits (`Active` or
  `FinishedThisFrame`, as `EventEmissionSystem` gates), whether its crossfade source is ghosted
  (the `Blending` flag), and its loop-resolved playhead.
- `actor-profiles.md`: "Layer events" and "Which layers emit events" paragraphs.
