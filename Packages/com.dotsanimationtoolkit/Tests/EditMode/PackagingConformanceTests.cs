...
namespace DotsAnimationToolkit.Tests.EditMode
{
    ...
    public sealed class PackagingConformanceTests
    {
        ...
        private static readonly HashSet<string> PlainNounStaticClasses = new HashSet<string>
        {
            ...
            "ToolkitPalette", "ToolkitIcons", "ToolkitGlyphs", "CaptionDragHandle", "ToolkitChrome",
        ...
        public void Supplementary_PackageManifest_MatchesSection11Identity()
        {
            ...
            // -- not drift because someone edited the manifest. 0.60.0 is the timeline tabs pass (Clip Editor, Actor Profiles with hover play and a real asset bar, and the cutscene panel); 0.59.0 is the preview tabs pass (Retarget, VAT Bake, Capture, Ragdoll to its reference composition, Stats and Health); 0.58.0 is the asset tabs pass (Texture Packer without a required recipe, Flipbooks, Clip Sets, Rigs, Materials and Events); 0.57.0 is chrome consistency pass 2 (card gutters, one button family, one-tone icons and fifteen drawn tab glyphs); 0.56.0 is the style foundation (shared token and component sheets, the tab list, the neutral primary and the end of the magenta proxies); 0.55.0 is unified authoring and the whole-animation VAT preview; 0.54.0 is release readiness (Conformance_A green, the Samples~ conformance fixture, the three before-1.0 checks); 0.53.1 moves Ragdoll after Actor Profiles in the tab strip; 0.53.0 is the Stats tab; 0.52.1 renames Sprite Sheets to Flipbooks and Cutscene Director to Cutscenes; 0.52.0 is one chrome across every tab and the Conformance_I inline-style ratchet; 0.51.0 is the Ragdoll tab; 0.50.0 is a Skipped row adding its tag to a rig part; 0.49.0 is Create and assign on the Materials tab; 0.48.1 is the tab strip's owner order and full-width fill; 0.48.0 is the Capture tab's PNG sequences and GIFs; 0.47.0 is the Retarget tab; 0.46.0 is the Materials tab and the shader contract as data; 0.45.0 is Sprite Sheets over the project's existing Texture2DArrays; 0.44.0 is the two-panel Health tab and its error count on the tab; 0.43.0 is the Events tab's Used by column, with event routing removed; 0.42.0 is the Sprite Sheets tab and frame picking by name; 0.41.0 is the Health tab; 0.40.0 is the Events tab and event routing data; 0.39.0 is project-wide refactor operations; 0.38.0 is the layered event strip in the Actor Editor; 0.37.0 is the missing camera data warning and the Camera Sync sample; 0.36.0 is animation name errors reported at profile save and player build; 0.35.0 is stale VAT bake detection; 0.34.0 is scrub crossings and sound on scrub; 0.33.0 is one event inspector, pin and rule set for clip and cutscene markers; 0.32.0 the event payload schema behind the Clip Editor's payload dropdown; 0.31.0 the asset reference index behind every delete confirmation; 0.30.0 was the Clip Editor window decomposed into four panes; 0.29.0 the shared catalog column and the
            // cover-pane split that remembers its divider across a tab hide/show.
            Assert.AreEqual("0.60.0", manifest.version, "Version tracks the shipped feature set; 0.60.0 is the timeline tabs pass.");
