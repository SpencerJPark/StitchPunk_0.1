...
> **Status:** ✅ built 2026-09-10 as `0.28.0` — T0–T17 done, gated (EditMode 823, PlayMode 283); no capture (Editor unfocused); ⏸ T18 owner checkpoint open. Build log in §7.
...
- **T0 (2026-09-10).** `git status` at start: only `Assets/_Vault/Tasks/NewPlans/TexturePacker_Roadmap.md.meta` untracked (the owner's). Baseline: EditMode **820** discovered, 2 pre-existing failures (the standing `Conformance_A` asmdef drift, and `Supplementary_PackageManifest_MatchesSection11Identity` still expecting `0.26.0` against the shipped `0.27.0` — A80 never moved it; T15 moves it to `0.28.0` with the bump); PlayMode **283/283**. The game-side window **was driven**, against three generated 64×64 greyscale PNGs in `Assets/A81Scratch/` (`T_SrcA` = 50, `T_SrcB` = 200, `T_SrcC` = 120): `AddSourceNode` ×2, `ConnectPorts` (A's R → output G), `Bake` to `T_Probe.png` → file written, imported `RGB24`, uncompressed, sRGB off, pixel (10,10) reads `(0, 50, 0, 255)` — the packing itself works. Two findings: **(1) the toolbar is drawn over by the graph** — `graphView.layout` is `(0, 0, 1281, 636)` with `position = Absolute` while the toolbar sits at `(0, 0, 1281, 21)`; the root's 2nd child covers its 1st, so D19's `graphHost` is required, not optional. **(2) programmatic `AddSourceNode`/`ConnectPorts` raise no `GraphChanged`** — only `graphViewChanged` (user edits) and the drop handler do, so `AutoAssignResolution` never ran for the reflected adds (the bake came out 1024² from 64² sources); the new helpers in §4.7 raise `GraphChanged` explicitly, which closes this for the sidebar paths. The verification checklist's July boxes are therefore answered by this drive.
- **T1 (2026-09-10).** `Editor/TexturePacker/PackRequest.cs` written by hand; `PackChannelIndex` fails `Conformance_G` until T15's allowlist entry lands (expected, seen in the baseline run).
- **Wave (2026-09-10).** Fourteen `worker`s spawned at once from the repo root (T2, T3, T4a, T4b, T5–T14), 54k–90k tokens each, none capped, no read-guard denials. One compile error across the whole wave: `PackOutputNodeView.cs` lacked `using UnityEditor;` for `DragAndDrop`. Surfaces matched the spec on every file (checked by grep before the gate). Commit `1a2b4287`.
- **T15/T16 (2026-09-10).** As §4.11, plus `Supplementary_PackageManifest_MatchesSection11Identity` moved from `0.26.0` to `0.28.0` (it had been left behind by A80). Game folder `git rm`'d; `Editor.md` inventory row, GraphView-pattern intro, pixel-reading pattern and the packer section re-pointed at the package; `Shaders.md:168` menu path replaced; `verify-texturechannelpacker.md` stamped superseded and moved to `Tasks/Completed/`. Commit `63190d66`.
- **T17 gate (2026-09-10).** `TexturePackMathTests` (3), `ClipEditorLayoutTests`, `PackagingConformanceTests` green bar `Conformance_A`. Revert-to-fail: disabling the invert branch made `ComposePixels_RoutesInvertsAndFillsDefaults` fail (`Expected: 191 But was: 64`); restored. Full suites: EditMode **823/823** (820 + 3; the third test is the missing-source path split out of the second, per T2's "separate method is fine"), PlayMode **283/283**.
- **T17 drive (2026-09-10)** — every step in §5 T17.2 passed, against `Assets/A81Scratch/T_SrcA/B/C.png` (64², values 50/200/120):
  - Tab: `images-list.itemsSource.Count` 75 = 75 `Assets/`-rooted `Texture2D`s; sidebar 280 × graph column 712 (window body 993); header 23 px above a 578 px graph host — the D19 host holds; the four header/sidebar buttons and the two mode toggles all resolve; `OutputNode` on the canvas at (620, 180).
  - `RaiseImagesActivated(T_SrcA)` → 1 source, `IsOnCanvas` true; eye toggle on → the row leaves `filteredImages`.
  - `AddSourcesWiredIntoChannel(Green, [T_SrcB])` twice → G port `connected`, **1** edge, 1 edge total.
  - Bake to `T_Packed.png` → exists, `RGB24`, sRGB off, uncompressed, resolution auto-assigned **64×64** (T0's finding fixed by the helpers raising `GraphChanged`), pixel (5,5) = `(0, 200, 0, 255)`: G carries B's value, **R is the unwired default 0** (A was on the canvas but not wired). Re-bake → same GUID.
  - `CreateRecipe("Assets/A81Scratch/Probe_Recipe.asset")` → `LoadRecipe` → selected in the catalog, sidebar on Recipes, 0 sources, label `Recipe: Probe_Recipe`. Wire A into R, bake to `T_Packed2.png` → label `Recipe: Probe_Recipe ●`; **the `.asset` on disk still had `outputAssetPath` empty and 0 wired channels.** `RaiseSaveRequested` → on disk 1 wired + `outputAssetPath: Assets/A81Scratch/T_Packed2.png`, label without ` ●`, `WiredChannelCount == 1`.
  - `RenameRecipe` → catalog reads `Probe_Recipe_Renamed`, graph unchanged; `AssetDatabase.OpenAsset` from the Clip Editor tab → `activeTab == TexturePacker`, `LoadedRecipe` is the recipe; `TrashRecipe` → catalog empty, canvas keeps its node, file gone.
  - G chip on node A → `ViewedChannel 1`, thumbnail is a fresh 64² texture reading `(50, 50, 50, 255)`; chip off → thumbnail is `SourceTexture` again, `ViewedChannel -1`.
  - `ReplaceSourceTexture(A, T_SrcC)` → true, GUID and title are C's, both ends of the R edge still `connected`, `WriteToRecipe` records C's GUID in R; with B added, `ReplaceSourceTexture(A, T_SrcB)` → false, GUID unchanged, console: `[DOTS Animation Toolkit] Texture Packer: that texture is already on another node.`
- **Three fixes the drive found**, all orchestrator edits after the wave: (1) `TexturePackerSidebar` placed the two mode toggles as loose children of the space-between `toolkit-pane-header`, which spread them to opposite edges — grouped in `sidebar-mode-toggles`, actions slot `marginLeft: auto`; the shared header's `flex-wrap` still puts the actions on a second row at 280 px (see HANDOFF observation). (2) `OnRecipeDeleteRequested` compared with Unity `==` and left `hasUnsavedChanges` false — now `ReferenceEquals` and the canvas is marked unsaved, so the label reads `No recipe ●` as §5 expects (the dialog path itself cannot be driven). (3) `OnRecipeRenameRequested` never refreshed the header label — added `RefreshRecipeLabel()`.
- **Not done: the capture.** `EditorApplication.isFocused` was false on every attempt (the owner was in another application), and the vault forbids saving a stale frame or stealing focus. Nobody has looked at the tab. **Drift noted:** none — every line range in §3 matched on the day.
