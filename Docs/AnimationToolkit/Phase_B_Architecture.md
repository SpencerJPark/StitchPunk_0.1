...
| `StitchPunk.AnimationToolkit.Tests.PlayMode` | Runtime, Authoring, UnityEngine.TestRunner, Unity.Entities, Unity.Entities.Hybrid, Unity.Collections, Unity.Mathematics, Unity.Mathematics.Extensions, Unity.Burst, Unity.Transforms | **["Editor"] only** (amendment A17) | World/system integration tests (§11). |
...
**Recorded without the owner in the room.** The C3 handoff put this decision to the product owner with two coherent options — (a) bless the managed-baker location, (b) revert to spec and report from the Bursted pass — and a recommendation of (a). The owner was away for the session that closed C3, having asked for work to continue without them. (a) is recorded here because it is the recommended option, because it is what the shipped code already does, and because the alternative was to leave a known spec/reality contradiction standing, which is the failure mode that sank C1, C2 and C3. **If the owner prefers (b), this amendment and the `RigTargetBaker` branch it blesses are the whole of what must be reverted** — the `ActorBakeFailed` tag stands either way, since it fixes a defect that exists under both options.
