...
1. **Registry header** — `schemaVersion` (int32), `setKey` (uint64), `vatSetKey` (uint64), `layerCount` (byte).
2. **Target block** — `sortedTargetIds.Length` (int32), then **per dense target, interleaved**: its id (uint32) followed by the three `asuint` components of its `targetBoundsExtents`. One count covers both arrays; they are parallel and always the same length, so a second count would be redundant.
3. **VAT info** — `flavor` (byte), `textureWidth` (int32), `rowsPerFrame` (int32), `boneOrVertexCount` (int32).
4. **Clip count** (int32), then **per clip in ascending-`clipId` order**: `clipId` (uint64); `debugName` length (int32) then its bytes; `asuint(duration)`; `defaultLoop` (byte); `asuint(defaultBlendIn)`; `asuint(defaultBlendOut)`; **transform tracks** (count int32, then per track `targetIndex` int32, `blendOp` byte, `channels` byte, key count int32, then per key the `asuint` bits of `normalizedTime`, `position.xyz`, `rotationZ`, `scale.xy` plus `interpolation` byte); **sprite tracks** (count int32, then per track `targetIndex` int32, `mode` byte, key count int32, then per key `asuint(normalizedTime)`, `sliceIndex` int32, and the four `asuint` lanes of `atlasRect`); **events** (count int32, then per marker its fields); and finally `vatFrameStart` (int32), `vatFrameCount` (int32), `asuint(vatFps)`, and `offsetBounds` Center then Extents as `asuint` components.
