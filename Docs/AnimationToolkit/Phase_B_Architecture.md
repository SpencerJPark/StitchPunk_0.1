...
3. **Content hash.** `Unity.Collections.xxHash3` (the collections package's 64-bit xxHash3 — no managed hashing) over a canonical stream covering **every field of the finished blob**. Floats are hashed **by bit pattern** via `math.asuint` — same input bits, same hash, no re-quantization. The `Unity.Entities.Hash128` passed to `AddBlobAssetWithCustomHash` is `new Hash128(lo32(contentHash), hi32(contentHash), (uint)schemaVersion, lo32(setKey) ^ hi32(setKey))` — **this is the BlobAssetStore dedup key**. Two actors sharing a `ClipSetAsset` share one blob; re-bakes with unchanged content are no-ops. Because the folded `setKey` is part of the dedup key *and* `setKey` is in the hashed stream, dedup is scoped to a single set: two different sets never collapse onto one blob even with identical content.
   ...
   **Amendment A10 (C2 gate, 2026-07-29 — product-owner approved): coverage.** The previously normative append order omitted `sortedTargetIds`, `targetBoundsExtents`, every `vatInfo` field, and `ClipBlob.debugName` — all of which are *in the blob the hash keys*. Since the hash is the dedup key, a change confined to those fields returned a **stale blob**: rebaking VAT textures to a new `textureWidth` with unchanged frame ranges is the concrete case. The stream is now, in order: the registry header (`schemaVersion` int32, `setKey` uint64, `vatSetKey` uint64, `layerCount` byte); `sortedTargetIds` (count int32, then each uint32); `targetBoundsExtents` (count int32, then each component as `asuint`); `vatInfo` (`flavor` byte, `textureWidth` int32, `rowsPerFrame` int32, `boneOrVertexCount` int32); then per clip **in ascending-`clipId` order**: `clipId` (uint64), `debugName` (length int32 then its UTF-8 bytes), `math.asuint(duration)`, `defaultLoop` (byte), `math.asuint(defaultBlendIn)`, `math.asuint(defaultBlendOut)`, `vatFrameStart` (int32), `vatFrameCount` (int32), `math.asuint(vatFps)`, `offsetBounds` (Center and Extents as `asuint` components); then transform tracks in canonical order — per track `targetIndex` (int32), `blendOp` (byte), `channels` (byte), key count (int32), then per key the `math.asuint` bits of `normalizedTime`, `position.xyz`, `rotationZ`, `scale.xy` plus `interpolation` (byte); then sprite tracks and events in the same per-field style, each preceded by its count. **Every array is preceded by its element count**, so a boundary shift cannot alias. Any change to this order or coverage is a format change and **must** bump `schemaVersion` (now **2**). A conformance fixture pins a known set's hash to a literal constant so a Collections-side change cannot silently re-validate itself while invalidating every baked subscene.
...
Per clip: `offsetBounds` = union over transform tracks of, per key, `position.xy` offset ⊕ target's `boundsExtents` scaled by `max(|scale.x|, |scale.y|, 1)`; VAT clips union their `VatClipRange.bounds`. Conservative by construction (keys bound the extremes of all five interpolation modes because every easing here is monotonic between keys). This answers audit open question 13: **yes, bake-time conservative bounds**.
...
Resolution: the blob's `offsetBounds` is normatively **offset space**, and the entity-baking step (§4.1 / M2), which does see rest poses, is responsible for producing actor-space bounds by combining `offsetBounds` with the rest-pose positions of the targets each clip touches. That result is carried by `ActorRestBounds` (§5.2) and consumed by `RenderBoundsUpdateSystem` (§5.8). No runtime system may treat `offsetBounds` as actor space.
