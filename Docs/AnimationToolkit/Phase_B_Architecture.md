...
**Amendment A102 (release readiness, 2026-09-15): the Editor assembly references `Unity.RenderPipelines.Universal.Runtime`.** The cutscene viewport (`CutsceneViewportElement`) and the capture source (`CutsceneCaptureSource`) use `UnityEngine.Rendering.Universal`; `package.json` has declared the `com.unity.render-pipelines.universal` dependency throughout, so the reference adds no new requirement. The row lagged the asmdef, and `PackagingConformanceTests` Conformance_A reported it as a standing failure for months — which is why the row is corrected rather than the reference removed. The reference is placed last in the list.

Shader folders carry no asmdef (no C#). Samples carry their own small asmdefs referencing package assemblies plus the Unity assemblies they use, never an assembly the package's own asmdefs do not reference or define (enforced on disk by `SamplesCompileConformanceTests`, amendment A102).
...
ClipAsset ──contains──> TransformTracks / SpriteTracks / EventMarkers / VatSource
   ▲
   │ registered in
ActorAuthoring ─names──────> RigAsset + N ClipSetAssets          (Phase F: the bind)
ClipSetAsset ──references──> N ClipAssets + (optional) VatTextureSetAsset
   ▲                                                        ▲
...
| **Clip set** | `ClipSetAsset` | A rig-agnostic collection of motion: clips + optional VAT texture set (Phase F removed its `rig`). Replaces the audit's `AnimationLibrarySO` (audit §8: Replace — honored). A rig plus the sets bound to it bakes to one `ClipRegistryBlob`. |
...
```csharp
public sealed class ClipSetAsset : ScriptableObject            // Phase F: rig-agnostic motion
{
    [SerializeField] internal ulong stableId;
    // No rig field of any kind. A set is paired with one only on an ActorAuthoring.
    public List<ClipAsset> clips;                             // any rig; unaligned tracks skip (T2/T6)
...
public sealed class VatTextureSetAsset : ScriptableObject     // GENERATED by VatTextureBaker; fields read-only in inspector
{
    ...
    public ulong sourceRigKey;                                // Phase F: RigAsset.StableId this was baked from (V40)
...
| V02 | Error | A **VAT** track's `targetId` is not present in the bound rig (Phase F narrowed this from all tracks; VAT cannot retarget) |
...
| V05 | Error | Duplicate `ClipId` across the clip sets bound to one actor; duplicate `TargetId` inside a `RigAsset` |
| V06 | — | **Retired (Phase F).** A clip set names no rig. The number is not reused. |
...
| V11 | Warning | Clip registered twice across the sets bound to one actor (deduped at bake) |
...
| V38 | Warning | Rule T6 (Phase F): track `targetId` not declared by the rig of this bind — skipped, not failed |
| V39 | Error | Two clip sets bound to one actor each supply a `VatTextureSetAsset`; a registry addresses one |
| V40 | Error | A bound set's `vatTextures.sourceRigKey` names a rig other than the one bound; VAT cannot retarget |
...
|---|---|---|---|
| `ActorAuthoring` + `ActorBaker` | MonoBehaviour + `Baker<ActorAuthoring>` | Authoring | Root entity: builds the `ClipRegistryBlob` from the actor's own rig and its list of `ClipSetAsset`s (via `ClipRegistryBuilder`), registers it with `AddBlobAssetWithCustomHash`, adds all root components/buffers (§5.2), seeds starting layer state from the merged union. `DependsOn` the rig, every set, every clip, and the VAT texture set so edits retrigger baking. |
...
public struct ClipRegistryBlob
{
    ...
    public ulong setKey;                          // Phase F bind key: rig.StableId XOR every bound set's
...
- **EXPOSES:** all of the above types public (asset fields public per sketches; `stableId` fields internal with `[InternalsVisibleTo]` for Editor + Tests asmdefs); `ClipRegistryBuilder.Build(RigAsset, IReadOnlyList<ClipSetAsset>, out BlobAssetReference<ClipRegistryBlob>, out Unity.Entities.Hash128 contentHash)`; `ClipRegistryBuilder.TryComputeContentHash(RigAsset, IReadOnlyList<ClipSetAsset>, out Unity.Entities.Hash128 contentHash)` (**Phase F** — one blob per (rig, set-list) bind) and `ClipRegistryBuilder.SchemaVersion` (**amendment A16**, C2 gate — the dedup key without a blob to own, so a baker can probe the `BlobAssetStore` before deciding to build; without this on the EXPOSES list M2's baker cannot legally call the API §4.5 documents it around, and the canonical `TryGet`/build/`TryAdd` pattern is unreachable without either leaking a discarded blob or hand-disposing one the store is documented to own); `IStableIdMintReporter` with `HasUnpersistedStableId` / `MarkStableIdPersisted` (**amendment A16** — the §3.4/A14 contract by which the authoring layer reports a minted id for the editor layer to persist; M5 is its intended consumer); `ClipValidation.ValidateClip / ValidateBind / ValidateRig`; `ValidationMessage` with its `assetContext` object, `ValidationCode`, `ValidationSeverity`, `ValidationStage`, and `ClipValidationException` with its `Messages` list (**amendment A16** — a consumer cannot act on a finding or catch a failed bake without these).
