...
| 29 | Nothing outside the package modified | §1.2, review scope | **PASS** | `git diff --stat ec44226..HEAD` is 22 files, all under `Packages/com.dotsanimationtoolkit/`. `git status --short` on the package is empty. The host-repo modifications in the working tree predate C3 and are untouched by these commits. |
...
| **A3** | **Closed — no defect; behaviour pinned** | Checked against the Entities source rather than assumed: `Baker.GetComponentsInChildren<T>` defaults to `kDefaultIncludeInactive = true` and offers **no** bool overload, unlike the `GameObject` method it is named after. The two passes therefore already agree, and the advisory's "if those defaults exclude inactive children" premise does not hold. The first attempt at this entry changed the call to `GetComponentsInChildren<RigTargetAuthoring>(true)` — which would not have compiled and would have been unnecessary if it had. `ADisabledPart_IsStillCoveredByTheRestBounds_BecauseItIsStillBound` now pins the agreement, and both the baker comment and the test say why the default is load-bearing, since it is an easy thing to "fix" into a real defect. |
