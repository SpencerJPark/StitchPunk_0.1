...
- [ ] On rig parts whose packed mask genuinely uses the G/B layers (e.g. mouth with blood overlay): tick `BodyPartAuthoring.useLayerChannels`. Leave it off everywhere else — off bakes the secondary/tertiary tints at alpha 0 so stray G/B data (greyscale sources copied into all channels, the head artefact) can never composite. Palette designs that write those slots re-activate the layers on their own.
