...
- **Commit and push to `main` whenever it makes sense — do not wait to be asked** (owner, 2026-08-02). What has not changed: **stage paths explicitly, never `git add -A`.** The working tree carries substantial unrelated host-game shader work (Painterly graphs, colour ramps, `Assets/Shaders/`) that must never ride along. Stage `Packages/com.dotsanimationtoolkit`, `Docs/AnimationToolkit`, and this file by name.
