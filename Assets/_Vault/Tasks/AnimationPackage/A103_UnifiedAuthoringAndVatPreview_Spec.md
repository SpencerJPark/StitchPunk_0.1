...
15. A `[Serializable]` class field (`ClipAsset.vatSource`) deserializes non-null, so D1's "a `vatSource`" is implemented as `vatSource != null && vatSource.sourceClip != null`; an in-memory `CreateInstance` clip keeps it null, which is what F5 asserts.
...
**W1 (T1–T6)** — stub `f36e2260`, wave `7e7a57b4`. Gate 1: compile error CS0540 in `ImportedClipLaneResolverTests.cs:56` (explicit `IComparer.Compare` without declaring the interface); fixed by a fresh worker.
