...
**W2 (T7–T11)** — wave `b1199d04`. Drift found while building:
16. `RigTargetDefinition` has no rest-slice field; `VatPreviewPartPoser` reads `RigTargetAuthoring.restSliceIndex` off the node (0 when absent), the same source `CutscenePreviewController.CapturePartBindings` uses.
17. `PreviewRigMirror` has no has-target member; the controller's `TryGetRestPose` gates on `GetPartTransform(targetId) != null`.
