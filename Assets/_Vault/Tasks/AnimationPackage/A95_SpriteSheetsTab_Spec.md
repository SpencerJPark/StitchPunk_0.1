...
- **Wave gates.** Wave commit `6693ee01` (nine workers, T2-T10). First gate refused ("Unity is compiling"); the retry failed to compile on two missing usings (`System.Linq` for `IReadOnlyCollection.Contains` in `SpriteSheetValidation`, `UnityEditor` for `DragAndDrop` in `SpriteSheetFramesColumn`), fixed by a fresh worker in `149c4bd4`. Gate at `149c4bd4` with namespace-qualified fixtures: **pass**, compile clean, **6/6** (`SpriteSheetValidationTests` 2, `SpriteSheetBakerTests` 1, `ClipEditorAddEventTests` 3).
- **Gate trap (stage, broker `TestCapture.cs:28`).** The broker matches `^<name>(\.|$)` against the full test name, so a bare fixture name matches nothing yet reports pass with "passed 1". The T1 gate at `3b36f652` used a bare name and proved only compile; the qualified wave gate supersedes it. Always pass `DotsAnimationToolkit.Tests.EditMode.<Fixture>` and check that passed equals the fixture total.
- **Revert-to-fail.** Mutation commit `351dfedf`, alone: the baker writes layers in reverse, `FindSizeMismatches` returns on the first offender, and `DedupeFrameName` drops the counter. Gate: **test-failures, 3 passed / 3 failed**, exactly the three sprite-sheet tests (`Bake_LayerOrderIsListOrder` expected 255 but was 0; `DedupeFrameName_AppendsCounter` "head 2" vs "head 1"; `SizeMismatches_NameEveryOffender` 2 vs 1). `git reset --hard HEAD~1` back to `149c4bd4`; sha256 matches the pre-mutation files (baker `1C0FFFB5…777AA5`, validation `F2D80D2E…223C89`).
- **Unverified in the worktree (needs the stage drive):** every UI surface (panel, columns, contact sheet, inspector pickers) has only compiled, never been shown; ListView `reorderable` / `itemIndexChanged` have no earlier use in the package; the `CopySerialized` overwrite-in-place path; byte-exact decode of real project PNGs; the working copy's Save round-trip.
