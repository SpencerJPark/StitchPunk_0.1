...
> **Status:** ✅ decisions locked (2026-08-29) — ready to build, pending phase-0 prereqs (see §9)
...
LateSimulation: Spawn → SpawnInit → Ragdoll (toolkit `RagdollActor`, replaces Ragdoll2D — §5) → Sound → Despawn → Save
```
...
- The toolkit's ragdoll presentation sub-group runs where `Ragdoll2DSystem` used to (LateSimulation, after all of `SimulationSystemGroup`), so the "apply-pose stomps, ragdoll re-writes after" contract survives with **no change** — the toolkit's ragdoll apply step replaces `ApplyAnimatedPoseSystem`/`Ragdoll2DSystem` as the stomper.
...
**Rewritten:** `UnitAnimationAssignmentSystem.cs` · `DesignApplyUtil.cs` (+`DesignApplySystem`/`DesignChangeSystem` write targets) · `Systems/GameManagerSystemGroup/CorpseCellSystem.cs` (§5, re-wire onto `RagdollActor`/`RagdollLaunch`) · `Systems/HealthSystemGroup/DeathSystem.cs` (§5, `Health.kill*` seeding onto toolkit ragdoll)
