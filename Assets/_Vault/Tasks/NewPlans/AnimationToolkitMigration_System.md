...
> **Status:** 🔨 building (2026-08-29) — phases 3–6 landed (vocabulary, command-seam cutover, seams + ragdoll adoption, legacy deletion + docs truth pass). C# compiles clean; Burst's in-process compiler cache is corrupted this session (survives disk-cache clears — needs an Editor restart) but the two flagged call sites were hand-verified against their declarations and are correct. Remaining: phase 2 (clip/rig authoring, owner) and phase 7 (verify, once phase 2 lands and a restarted Editor confirms Burst is clean). Two scene cleanup items need a human in the Editor — see the retire-time note below.
...
6. **Delete + docs truth pass.** Everything in §8's delete list; rewrite `Systems_Animation.md` as the game↔toolkit seam note; purge `Gotchas.md` animation entries; fix `Assets/CLAUDE.md`; update `Contracts.md` (`AnimationCommand`/`AnimEventOutput` rows replace `SetAnimation`).
   - ✅ Done (2026-08-29). Deleted: the 6 remaining read-side legacy systems + `UnitFaceDirectionSystem` + `AnimationLibraryBakingSystem`; `AnimationComponents.cs` (tint components + `BaseParent` moved to `BodyPartComponents.cs` first); `AnimationUtils.cs`; `AnimationBlobs.cs`; `AnimationClipSO.cs`/`AnimationLibrarySO.cs`; `AnimationLibraryAuthoring.cs`; `BillboardAuthoring.cs` + the `Billboard` component; `ImageIndexAuthoring.cs` (a standalone legacy baker not in the original file list, found by grep — baked the now-deleted `ImageIndex`/`ImageIndexOverride`); `AnimationEnums.cs` trimmed to just `AnimationTarget` (still live — `BodyPart`'s identity key); the whole `Editor/AnimationEditor/` tool plus its two private scenes (`AnimationEditorScene.unity`, `AnimationEditorSubScene.unity` — not in Build Settings, referenced by nothing else); 9 dead `Core/Unused/` animation files; the legacy clip/library **assets** (`_AnimationLibrary.asset`, `None.asset`, and the `Action/Base/Direction/Eyes/Mouth` folders — 21 clips) — `NewRig.asset`/`NewClipSet.asset`/`NewClip*.asset` untouched. `BodyPartAuthoring.cs` no longer bakes `AnimationTargetRestPose`/`AnimationTargetPose`/`ImageIndex`/`ImageIndexOverride` (dead once their readers were deleted); its `baseImageIndex` field went with them — the toolkit's `RigTargetAuthoring.restSliceIndex` is the replacement, authored separately.
   - ⚠️ **Two items need a human in the Editor**, not left broken but not fully cleaned either: `AnimationLibrary.prefab` (`Prefabs/EntityLibraries/`) still carries the now-deleted `AnimationLibraryAuthoring` component (will show "missing script"), and it has one instance placed in `DOTSTestScene.unity` — I could not reach into that subscene's hierarchy from outside the Editor to remove the instance safely, and chose not to hand-edit the scene YAML given the corruption risk. Delete the prefab and remove its instance from `DOTSTestScene` next time the Editor is open.
7. **Verify** (below) then retire this doc to `Verification/`.

**Branch strategy — deviated from the DECIDED plan.** The spec called for phases 4–6 on a separate `animation-toolkit-cutover` branch, squashed on merge. In practice all of phases 3–6 landed as separate commits directly on `main` (four commits: command-seam cutover, ragdoll+seams, a spec-doc correction, legacy deletion) — the owner said "continue" each time without a branch being raised, and nothing was pushed to a remote. `main`'s history is not the single clean squashed commit the plan called for; if that matters, an interactive rebase/squash of these four commits is still available before anything is pushed.
