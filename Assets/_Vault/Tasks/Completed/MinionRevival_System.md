...
> **Status:** 🔨 built (2026-06-14) · Phases 1–4 (all C#) landed; Phase 5 persistence is verify-only. See `verify-minion-revival.md`. Resolved decisions: same-frame swap · zero-decay zombie needs · `SwapBrainSystem` self-fires its interrupt · trust per-frame `FactionRegistry` rebuild · restore-combat-data is a reserved Phase-5 hook.
