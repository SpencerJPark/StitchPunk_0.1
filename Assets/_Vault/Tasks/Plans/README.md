...
| **Cutscene Roadmap** (finish the toolkit cutscene feature: A61–A68 package amendments in `Docs/AnimationToolkit/` + G1–G3 game plans; Sonnet-executed, one spec per session) | [Cutscene_Roadmap.md](../NewPlans/Cutscene_Roadmap.md) | ✅ specs written 2026-09-04 — nothing built; the old `Cutscene_System.md` was deleted as superseded |
| **Cutscene Integration** (G1: `CutsceneSystemGroup`, `CutsceneRequest` signal, `CutsceneActor` gates, Cinemachine bridge, `PlayCutsceneAction` — first in-game cutscene) | [CutsceneIntegration_System.md](../NewPlans/CutsceneIntegration_System.md) | ✅ spec ready — needs toolkit A61 + A62 first |
| **Cutscene Interactions** (G2: marks → `MovementAPI`, dialogue cue ↔ `ActiveDialogue`, `CutsceneFacing` → `UnitFacing`, detach → `ThrownItemRequest`) | [CutsceneInteractions_System.md](../NewPlans/CutsceneInteractions_System.md) | ✅ spec ready — needs G1 + toolkit A63–A65 |
...
| **Actor Editor Roadmap** (A70 actor profile + named playback + per-animation direction + ragdoll triggers; A71 the Actor Editor tab with a composited multi-layer preview; G5 game cutover + MaleCitizen profile and clips) | [ActorEditor_Roadmap.md](../NewPlans/ActorEditor_Roadmap.md) | ✅ specs written 2026-09-07 — nothing built; owner product calls recorded; Sonnet-executed in order A70 → A71 → G5 |
