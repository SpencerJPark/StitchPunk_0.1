...
| **Animation Event Timing** (AnimEvents replace hitTime/WaitTime timers; WaitForAnimEvent/WaitForClipFinished commands; Pickup converted, combat Attack-event trigger wired) | [AnimationEventTiming_System.md](../Verification/AnimationEventTiming_System.md) | ◐ built 2026-08-29, awaiting play-test — [verify-animationeventtiming.md](../Verification/verify-animationeventtiming.md) (assignment shrink deferred; Attack event not yet authored on any clip) |
...
| **Direction & Facing** (toolkit adoption: `FacingResolver`/`PartFacing`/`AnimationDirections.Six` default; `UnitFacingSystem` + `DirectionalClipSet` + part view offsets; supersedes the two pre-toolkit Direction specs — deleted 2026-08-29, git keeps them; channel packing re-scoped to intra-slice state variants, own spec later; coverage-derived direction sets + the authoring tool, since reworked into the toolkit's 2D Direction Sets pane) | [DirectionFacing_System.md](DirectionFacing_System.md) | 🔨 phases 1–4 built 2026-08-29; phase 5 (art proof) now runs through the reworked panel below |
| **2D Direction Sets Panel** (rework of the §6a tool: `DirectionSetSO` promoted into the toolkit as `DirectionSetAsset`; in-Clip-Editor toggle pane replaces the standalone window; single viewer + direction slider + clip queue; host unit-context seam; `UnitSO` gains maxHealth/rig/clipSet) | [verify-directionsetspanel.md](../Verification/verify-directionsetspanel.md) | 🔨 built 2026-08-29 — compiles clean, touched fixtures green; **awaiting the owner's eyes** |
| **Clip Editor Tabs + Viewport Overlay** (top bar becomes four exclusive tabs, Clip Editor default; identity fields + Billboard/Ragdoll + new Move/Rotate/Scale buttons move into a floating right-aligned overlay over the preview; VAT Bake and 2D Direction Sets read the window's clip set + rig, the latter folding onto the window's single preview controller) | [verify-clipeditortabs.md](../Verification/verify-clipeditortabs.md) | 🔨 built 2026-08-29 — compiles clean, EditMode 765/765 + PlayMode 241/241; **awaiting the owner's eyes** |
...
| **Minion Order Robustness** (order-time attack resolution from AvailableAttack; Stop/ReturnToPlayer verbs) | [MinionOrderRobustness_System.md](../Verification/MinionOrderRobustness_System.md) | ◐ built 2026-09-15, awaiting play-test (rebake first) — [verify-minion-orders](../Verification/verify-minion-orders.md) |
| **Behavior Command Split** (extract interpreter switch arms into Utils/BehaviorCommands; pure refactor) | [BehaviorCommandSplit_System.md](../Verification/BehaviorCommandSplit_System.md) | 🔨 built — [verify-behaviorcommandsplit.md](../Verification/verify-behaviorcommandsplit.md) still open |
...
| **Movement Toolkit Extraction** (lift movement/pathfinding out of `_Scripts` into a sellable `com.dotsmovementtoolkit` UPM package, mirroring `com.dotsanimationtoolkit`) | [Movement_Toolkit_Extraction.md](Movement_Toolkit_Extraction.md) | 🔨 built 2026-08-29, all four phases; [verify-movement-toolkit.md](../Verification/verify-movement-toolkit.md) still open |
| **Cutscene Roadmap** (finish the toolkit cutscene feature: A61–A68 package amendments in `Docs/AnimationToolkit/` + G1–G3 game plans; Sonnet-executed, one spec per session) | [Cutscene_Roadmap.md](../NewPlans/Cutscene_Roadmap.md) | ✔️ A61–A73 built (package 0.19.0) and G1–G3 + G6 built; the old `Cutscene_System.md` was deleted as superseded; only the in-game checkpoints are open (audit 2026-09-15) |
| **Cutscene Integration** (G1: `CutsceneSystemGroup`, `CutsceneRequest` signal, `CutsceneActor` gates, Cinemachine bridge, `PlayCutsceneAction` — first in-game cutscene) | [CutsceneIntegration_System.md](../NewPlans/CutsceneIntegration_System.md) | 🔨 phases 1–5 built and gated 2026-09-04; ⏸ owner checkpoint open (nobody has watched a cutscene play in-game) |
| **Cutscene Interactions** (G2: marks → `MovementAPI`, dialogue cue ↔ `ActiveDialogue`, `CutsceneFacing` → `UnitFacing`, detach → `ThrownItemRequest`) | [CutsceneInteractions_System.md](../NewPlans/CutsceneInteractions_System.md) | 🔨 phases 1–5 built and gated 2026-09-06; ⏸ owner checkpoint open |
| **Cutscene Acceptance** (G3: the "Rendezvous and Depart" cutscene authored with real assets + `verify-cutscene.md`) | [CutsceneAcceptance_System.md](../Verification/CutsceneAcceptance_System.md) | 🔨 built 2026-09-07 — [verify-cutscene.md](../Verification/verify-cutscene.md) still open; last before the docs pass |
...
| **Actor Editor Roadmap** (A70 actor profile + named playback + per-animation direction + ragdoll triggers; A71 the Actor Editor tab with a composited multi-layer preview; G5 game cutover + MaleCitizen profile and clips) | [ActorEditor_Roadmap.md](../NewPlans/ActorEditor_Roadmap.md) | ✔️ A70 (0.16.0), A71 and G5 P1–P9 built; ⏸ G5-P10 in-game checkpoint open; owner product calls recorded |
| **Actor Profile Cutover** (G5: `UnitSO` → profile, name-convention binding, assignment by name, `ActorFacing`, deletes `DirectionSetBlob`/`AnimationToolkitLayer`, authors the first profile) | [ActorProfileCutover_System.md](../NewPlans/ActorProfileCutover_System.md) | 🔨 P1–P9 built 2026-09-07; ⏸ P10 owner checkpoint |
| **Animation Layers Content** (AL) | [AnimationLayersContent_System.md](../NewPlans/AnimationLayersContent_System.md) | ⛔ superseded 2026-09-07 by the Actor Editor roadmap; clip recipes live on in G5 |
| **Cutscene Profile Cutover** (G6: `CutsceneRequest.layerIndex` gone, assignment gated on `CutsceneActor`, facing-snap parity fixture, seven cutscene assets re-pointed at `MaleCitizen.profile` by script, F9 acceptance with walking minions) | [CutsceneProfileCutover_System.md](../NewPlans/CutsceneProfileCutover_System.md) | 🔨 P1–P4 built 2026-09-08 (toolkit A73 done first); P5 code-reviewed, empirical confirmation folded into the ⏸ P7 owner checkpoint; P6 (this row) done |
| **Ragdoll Bodies + Death Tuning** (RG: eleven ragdoll bodies on `NewRig`, baked `RagdollLaunch`, torque-axis + mass-scaling fix, preview/in-game/revive verification, launch tuning) | [RagdollTuning_System.md](../NewPlans/RagdollTuning_System.md) | 🔨 T1–T3 code built 2026-09-07; bodies authored; T4–T10 owner-paced |
