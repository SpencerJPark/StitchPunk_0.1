...
| **Color Palette** (global enum-indexed `ColorPaletteBlob`; per-part `PaletteColorRef` → `TintApplySystem` writes `BodyPartTint`/`_BaseColor`; linear-baked, one-shot `ApplyTintRequest`) | [ColorPalette_System.md](ColorPalette_System.md) | ✅ spec ready |
