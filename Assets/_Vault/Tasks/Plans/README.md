...
| **Direction & Facing** (toolkit adoption: `FacingResolver`/`PartFacing`/`AnimationDirections.Six` default; `UnitFacingSystem` + `DirectionalClipSet` + part view offsets; supersedes the two pre-toolkit Direction specs — deleted 2026-08-29, git keeps them; channel packing re-scoped to intra-slice state variants, own spec later; `DirectionSetSO` coverage-derived sets + Direction Set Editor tool) | [DirectionFacing_System.md](DirectionFacing_System.md) | 🔨 phases 1–4 built 2026-08-29; phase 5 (art proof) now runs through the reworked panel below |
| **2D Direction Sets Panel** (rework of the §6a tool: `DirectionSetSO` promoted into the toolkit as `DirectionSetAsset`; in-Clip-Editor toggle pane replaces the standalone window; single viewer + direction slider + clip queue; host unit-context seam; `UnitSO` gains maxHealth/rig/clipSet) | [DirectionSetsPanel_System.md](../NewPlans/DirectionSetsPanel_System.md) | ✅ spec ready — foundations stamped 2026-08-29, build-time DECISIONs open |
