...
| SO | Enum Key | Purpose |
|---|---|---|
| `UnitLibrarySO` + `UnitSO` | `UnitType` | Unit stats, prefab refs, base motivation weights, `animationDirections` (turn granularity, default `Six`), `maxHealth` (spawn+max health, stamped onto the entity by `UnitHealthInitSystem` — the prefab's `HealthAuthoring` numbers are a fallback for scene-placed units only), and validate-only `rig`/`clipSet` (the prefab's `ActorAuthoring` stays the runtime source of truth; a disagreement warns via `UnitSO.DescribeRigMismatch`, it is never pushed onto the entity). Clip fields are `DirectionSetAsset`, not `ClipAsset`. Attack damage is **not** duplicated here — it stays on `AttackSO`, reached through `UnitSO.attacks` |
| `DirectionSetAsset` — **lives in the toolkit** (`Packages/com.dotsanimationtoolkit/Authoring/Assets/`, namespace `DotsAnimationToolkit.Authoring`), no library; referenced directly by `UnitSO`/mapping fields | — | Five east-side `ClipAsset` slots (`southEast/northEast/south/north/east`) + an authoring-intent `targetDirections`; effective `AnimationDirections` **derived** from fill pattern via `TryGetEffectiveDirections` (never declared). Bakes to the game-side `DirectionSetBlob` via `DirectionSetBakeUtil.Bake`, which warns on an invalid pattern (rounding down) and on a valid one authored below its target. Authored through the Clip Editor's **2D Direction Sets** pane. See `Systems_Animation.md` (facing) |
