...
| `ActorFacing` | `UnitFacingSystem` (`AnimationAssignmentSystemGroup`) — writes `.facing` alongside the game's own `UnitFacing` whenever the resolved direction changes, reading turn granularity from the actor's baked `ActorProfileBlob.turnDirections` | `com.dotsanimationtoolkit`'s `ActorFacingRepickSystem` — re-resolves the active directional clip on the affected layers against the new facing |
