...
| `CutsceneHoldRelease` (package) | `CutsceneDialogueCueSystem` when a cued `ActiveDialogue` closes | `CutsceneTimelineSystem` — releases the segment's hold. A holding event's id is the event's **registry name**; read it back with `CutscenePlaybackApi.TryGetCurrentHoldId` rather than assuming (G2) |
