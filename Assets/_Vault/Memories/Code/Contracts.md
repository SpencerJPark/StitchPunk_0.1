...
| `AnimEventOutput` (gated `AnimEventsPending`) | `com.dotsanimationtoolkit`'s `EventEmissionSystem` | `AnimEventSoundSystem` (`SoundSystemGroup`) — maps event keys to `SoundType` via `AnimSoundEventMappingSO`/blob. `BehaviorExecutionSystem` (`WaitForAnimEvent`/`WaitForClipFinished` commands, `Utils/BehaviorCommands/WaitForAnimEventCommand.cs`/`WaitForClipFinishedCommand.cs`) — per-actor buffer read, gated on the same `AnimEventsPending` enableable. `AttackRequestSystem` (`CombatExecutionSystemGroup`) — `AnimEvents.Attack` (the registry's key 18) on the Action layer triggers the `DamageBus` enqueue; `AttackBlob.hitTime` is the fallback/timeout when the event never arrives. All three read one frame after `EventEmissionSystem` emits (`AnimationToolkitSystemGroup` runs after `StateMachineSystemGroup`/`CombatSystemGroup` — documented 1-frame latency contract, see `Systems_AI.md`) |
