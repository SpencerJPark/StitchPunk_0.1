...
public class UnitSelectionManager : Singleton<UnitSelectionManager>, IUpdateObserver
{
    ...
    private void HandleMoveCommand(EntityManager entityManager, Vector2 pointerPos)
    {
        ...
        // Query for selected units with pathfinding components
        EntityQuery entityQuery = new EntityQueryBuilder(Allocator.Temp)
            .WithAll<Selected>()
            .WithPresent<PathfindingAgent, PathRequest, TargetOverride>()
            .Build(entityManager);

        NativeArray<Entity> entityArray = entityQuery.ToEntityArray(Allocator.Temp);
        NativeArray<PathfindingAgent> agentArray = entityQuery.ToComponentDataArray<PathfindingAgent>(Allocator.Temp);
        NativeArray<PathRequest> pathRequestArray = entityQuery.ToComponentDataArray<PathRequest>(Allocator.Temp);
        NativeArray<TargetOverride> targetOverrideArray = entityQuery.ToComponentDataArray<TargetOverride>(Allocator.Temp);
        
        // Generate spread-out positions for multiple units
        ...
        for (int i = 0; i < entityArray.Length; i++)
        {
            ...
            // Set up path request
            PathRequest pathRequest = pathRequestArray[i];
            pathRequest.targetPosition = movePositionArray[i];
            pathRequest.requestedMode = agentArray[i].preferredMode;
            pathRequestArray[i] = pathRequest;
            
            // Enable the path request component
            entityManager.SetComponentEnabled<PathRequest>(entity, true);
            
            // Update pathfinding agent
            PathfindingAgent agent = agentArray[i];
            agent.targetPosition = movePositionArray[i];
            agent.isActive = true;
            agent.needsRepath = true;
            agentArray[i] = agent;
            
            // Disable any active followers - they'll be re-enabled by the pathfinding systems
            if (entityManager.HasComponent<FlowFieldFollower>(entity))
            {
                entityManager.SetComponentEnabled<FlowFieldFollower>(entity, false);
            }
            if (entityManager.HasComponent<DStarLiteFollower>(entity))
            {
                entityManager.SetComponentEnabled<DStarLiteFollower>(entity, false);
            }
        }

        // Copy data back to entities
        entityQuery.CopyFromComponentDataArray(agentArray);
        entityQuery.CopyFromComponentDataArray(pathRequestArray);
        entityQuery.CopyFromComponentDataArray(targetOverrideArray);
