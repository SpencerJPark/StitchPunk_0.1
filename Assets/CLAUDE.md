...
- Legacy per-action systems (`MeleeContinuousActionSystem`, `TalkActionSystem`, etc.) are commented out in `ActionExecutionSystemGroup/` as reference semantics for the migration; remaining awareness stubs (`Schedule`, `Weather`) are pending later phases.
