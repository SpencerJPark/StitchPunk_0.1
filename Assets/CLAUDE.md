...
- The 8 legacy per-action systems were **deleted** from `ActionExecutionSystemGroup/` (semantics preserved in `_Vault/Memories/Code/Systems_AI.md`); remaining awareness stubs (`Schedule`, `Weather`, `Enviroment`) are pending later phases.

Behavior-recreation status (interrupts, self-defence, flee, talk, sit/pickup, minion orders, sound) is captured inline in this section and in [`_Vault/Memories/Code/Systems_AI.md`](_Vault/Memories/Code/Systems_AI.md); the only remaining awareness stubs are `Schedule` / `Weather` / `Enviroment` (see line above).
