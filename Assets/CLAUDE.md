...
- **Active:** the DOTS Animation Toolkit — `Packages/com.dotsanimationtoolkit`, specs in `_Vault/Tasks/AnimationPackage/`, docs in `Docs/AnimationToolkit/`. The game runs on it: `MaleCitizen` is the one toolkit actor (`_Scripts/Editor/ContentAuthoring/`), the legacy stack is deleted, `Systems_Animation.md` is the seam note.
